<script>
    import { getDicountPercent } from '../utils.mjs';
    export let product = {};
</script>
    ...
    <section>
        <a href="product_pages/index.html?id={product.Id}">
            {#if product.ListPrice !== product.FinalPrice}
                <span class="card__discount">-{getDicountPercent(product.FinalPrice, product.ListPrice)}%</span>
            {/if}
            ...
            {#if product.ListPrice !== product.FinalPrice}
                ...
                <p class="card__discount-price" id="productFinalPrice">${product.ListPrice}</p>
            {:else}
